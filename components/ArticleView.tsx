...
const TextInteractionModal = ({ text, mode, onClose, position }: { text: string, mode: 'Define' | 'Explain' | 'Visualize' | null, onClose: () => void, position: { top: number, left: number } }) => {
    ...
    useEffect(() => {
        if (!mode) return; // Only run this effect when the modal is active
    ...
    // FIX: The conditional return now happens *after* all hooks have been called.
    if (!mode || !text) {
        return null;
    }
        ...
        <div className={`prose prose-invert max-w-none prose-p:text-gray-300 prose-headings:text-gray-100 ${textSizeClass[settings.textSize]}`}>
            ...
            <p>{article.conclusion}</p>
             ...
             {interaction.text && !interaction.mode && (
                ...
                >
                    <button onClick={() => openInteractionModal('Define')} className="p-2 rounded-full hover:bg-gray-700" title={t('interaction.define')}><WandIcon className="w-5 h-5"/></button>
                    <button onClick={() => openInteractionModal('Explain')} className="p-2 rounded-full hover:bg-gray-700" title={t('interaction.explain')}><TextSelectIcon className="w-5 h-5"/></button>
                    <button onClick={() => openInteractionModal('Visualize')} className="p-2 rounded-full hover:bg-gray-700" title={t('interaction.visualize')}><ImageIcon className="w-5 h-5"/></button>
                </div>
            )}
